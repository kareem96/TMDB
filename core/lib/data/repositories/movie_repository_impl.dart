...
class MovieRepositoryImpl implements MovieRepository{
  ...
  @override
  Future<Either<Failure, List<Video>>> getTrailerVideo(int id) async{
    try{
      final result = await remoteDataSource.getTrailerVideo(id);
      return Right(result.map((e) => e.toEntity()).toList());
    }on ServerException{
      return Left(ServerFailure(""));
    }on SocketException{
  ...
  Future<Either<Failure, List<Review>>> getReview(int id) async{
    try{
      final result = await remoteDataSource.getReviews(id);
      return Right(result.map((e) => e.toEntity()).toList());
    }on ServerException{
      return Left(ServerFailure(""));
    }on SocketException{
