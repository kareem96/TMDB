...
abstract class MovieRemoteDataSource{
  ...
  Future<List<TrailerModel>> getTrailerVideo(int id);
...
class MovieRemoteDataSourceImpl implements MovieRemoteDataSource{
  ...
  Future<List<TrailerModel>> getTrailerVideo(int id) async {
    final response =
    await client.get(Uri.parse('$BASE_URL/movie/$id/videos?$API_KEY'));
    if (response.statusCode == 200) {
      return VideoResponse.fromJson(json.decode(response.body)).trailerList;
    }
    throw ServerException();
  }
  ...
  Future<List<ReviewModel>> getReviews(int id)async {
    final response = await client.get(Uri.parse("$BASE_URL/movie/$id/reviews?$API_KEY"));
    if(response.statusCode == 200){
